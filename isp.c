...
static void ispAttachToDevice(uchar stk500Delay, uchar stabDelay)
{
    ...
    if(PORT_PIN_VALUE(HWPIN_SS_JUMPER)==0){      /* Jumper is set -> request clock below 8 kHz */
    ...
    PORT_DDR_OUT(HWPIN_ISP_RESET);
    PORT_PIN_CLR(HWPIN_ISP_RESET);  /* activate RESET */
    /* setup initial condition: SCK, MOSI = 0 */
    PORT_OUT(HWPIN_ISP_SCK) &= ~((1 << PORT_BIT(HWPIN_ISP_SCK)) | (1 << PORT_BIT(HWPIN_ISP_MOSI)));
    PORT_DDR(HWPIN_ISP_SCK) |= (1 << PORT_BIT(HWPIN_ISP_SCK)) | (1 << PORT_BIT(HWPIN_ISP_MOSI));
...
uchar    ispReadMemory(stkReadFlashIsp_t *param, stkReadFlashIspResult_t *result, uchar isEeprom)
{
utilWord_t  numBytes;
uchar       *p, cmd0;
uint        i;
...
uchar    ispMulti(stkMultiIsp_t *param, stkMultiIspResult_t *result)
{
uchar   cnt1, i, *p;
