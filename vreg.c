...
#define CHANNEL_VREG    6
#define CHANNEL_TARGET  7
...
//#define DIVIDER (uchar)(22 * 1024 / ((33 + 22) * 25.6)) /* is coincidentally == 16 */ *1.1/4
...
/*    	if(abs(vREG_REF - value) > dU_MAX)
    		HVok = 0;
    	else
    		HVok = 1;
*/
        int x = PWM_CENTER + ((vREG_REF - value) << 5 );//*32
        if(x <= 0){
            x = 1;
        }else if(x > PWM_RANGE - (PWM_RANGE / 3)){//5
            x = PWM_RANGE - (PWM_RANGE / 3);
        }
        OCR1A = x;
         ...
         */
        stkParam.s.vTarget = (value * 9 ) >> 5;//*9/32 for 1/10 div
