 ...
 * Author: Christian Starkjohann, Windows HID stuff by Sjors Hettinga
...
#   define usbDevice_t  usb_dev_handle  // we prefer to use our own type
#endif
...
#define USB_VENDOR_RQ_DBG_ON    3
#define USB_VENDOR_RQ_DBG_OFF   4
...
#if !USE_WINDOWS_HID
static int  usbGetStringAscii(usbDevice_t *dev, int index, int langid, char *buf, int buflen)
{
char    buffer[256];
int     rval, i;
...
int usbOpenDevice(usbDevice_t **device, int vendor, char *vendorName, int product, char *productName, char dummy)
{
struct usb_bus      *bus;
struct usb_device   *dev;
usbDevice_t         *handle = NULL;
...
#define usbCloseDevice(device)  usb_close(device)

#endif

/* ------------------------------------------------------------------------- */

int openAvrDoper(usbDevice_t **device)
{
#if !USE_WINDOWS_HID
    if(usbOpenDevice(device, USBDEV_SHARED_VENDOR, USB_VENDOR_STRING, USBDEV_SHARED_CDCPID, USB_PRODUCT_STRING, 0) == 0)
            return 0;
#endif
    return usbOpenDevice(device, USBDEV_SHARED_VENDOR, USB_VENDOR_STRING, USBDEV_SHARED_HIDPID, USB_PRODUCT_STRING, 1);
}

void    avrDoperReconnect(usbDevice_t **device)
{
#if USE_WINDOWS_HID
    fprintf(stderr, "\nUSB error.\nTrying to reconnect...\n");
#else
...
// switched off because it also does not work on 64 bit systems
...
static int inputPoll(usbDevice_t **device)
{
int nBytes, c;
    ...
    while(_kbhit()){
		...
		if(c == 27) // ESC character
			...
			unsigned char c2[1];
			c2[0] = c;
			nBytes = usbSendData(device, c2, 1) == 0 ? 1 : -1;
#else
...
int             nBytes;
    ...
    usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, USB_VENDOR_RQ_DBG_ON, 0, 0, (char *)buffer, 0, 100);
	...
	printf("Any characters you type are sent to the device.\n");

    for(;;){
#if USE_WINDOWS_HID
        nBytes = reportDataSizes[USB_TYPE_HID_DEBUGDATA] + 2; // segment length + 1
    
        int usbErr = usbGetReport(handle,  USB_HID_REPORT_TYPE_FEATURE, USB_TYPE_HID_DEBUGDATA+1, (char *)buffer, &nBytes);
        if(usbErr != 0){
            fprintf(stderr, "avrdoper_get(): %s\n", usbErrorText(usbErr));
        ...
        if(nBytes > 0){
            normalizeAndPrintString(&(buffer[2]), nBytes);
        }
#else
    ...
    usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN, USB_VENDOR_RQ_DBG_OFF, 0, 0, (char *)buffer, 0, 100);
