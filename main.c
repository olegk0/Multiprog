...
*/
        ...
        case 2://receive
            ...
            break;
#if ENABLE_DEBUG_INTERFACE
        case 3:// Serial DBG On
        	serialInit();
        	break;
        case 4:// Serial DBG Off
        	serialStop();
#endif
        }
	}
...
//PORT_DDR_SET(HWPIN_UH_JUMPER);//output
//PORT_PIN_SET(HWPIN_UH_JUMPER);
PORT_PIN_CLR(HWPIN_UH_JUMPER);   /* deactivate pullup */
PORT_DDR_CLR(HWPIN_UH_JUMPER);//input
_delay_us(1);
useHIDInterface = PORT_PIN_VALUE(HWPIN_UH_JUMPER) != 0;
...
/*	PORT_PIN_CLR(HWPIN_SS_JUMPER);
	PORT_DDR_CLR(HWPIN_SS_JUMPER);//input
	_delay_us(1);
	useSlowSpeed = PORT_PIN_VALUE(HWPIN_SS_JUMPER) == 0;
*/

/*    UTIL_PBIT_SET(port, HWPIN_HVP_SUPPLY);
    UTIL_PBIT_SET(ddr, HWPIN_HVP_SUPPLY);
...
//    UTIL_PBIT_CLR(ddr, HWPIN_ADC_SMPS);
    UTIL_PBIT_CLR(port, HWPIN_HVP_HVRESET);
    UTIL_PBIT_SET(ddr, HWPIN_HVP_HVRESET);
...
//    UTIL_PBIT_CLR(ddr, HWPIN_ADC_VTARGET);
    ...
    UTIL_PBIT_SET(port, HWPIN_SC_JUMPER);
    UTIL_PBIT_CLR(ddr, HWPIN_SC_JUMPER);
*/
    ...
    // timer 0 configuration: ~ 1.365 ms interrupt @ 12 MHz  - for delay
   ...
   HVgenOff();
   TCCR1B = UTIL_BIN8(0000, 1001);  /* 9 bit, prescaler=1  20khz*/
